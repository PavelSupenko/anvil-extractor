...
class Compressor:
    ...
    def __init__(self):
        self.library_name = self._build_target_library_name()

    def decompress(self, mode: int, src: bytes, dst_len: int) -> bytes:
        """This is the function that does the actual decompression of the data"""
        lzo1x = {0, 1}

        lzo_library_name = self.library_name
        lzo_path = os.path.join(os.path.dirname(__file__), 'resources', lzo_library_name)

        try:
            lzo = CDLL(lzo_path)
        except OSError as exception:
            print(f'Failed to load lzo library by path "{lzo_path}". In that case you need to find an '
                  f'lzo library for your architecture, name it as target above.')
            raise exception
    ...
    def _build_target_library_name(self) -> str:
        lzo_library_name = 'lzo'
        ...
        is_x64 = architecture_bits == '64bit'
        is_x32 = architecture_bits == '32bit'
        is_arm = 'arm' in architecture

        if is_darwin:
            lzo_library_name += '-darwin'
        elif is_windows:
            lzo_library_name += '-win'
        else:
            lzo_library_name += f'-{system}'

        if is_arm:
            lzo_library_name += '-arm'

        if is_x64:
            lzo_library_name += '-x64'
        elif is_x32:
            lzo_library_name += '-x32'

        if is_darwin:
            lzo_library_name += '.dylib'
        elif is_windows:
            lzo_library_name += '.dll'

        print(f'Target library name: {lzo_library_name}')
        return lzo_library_name
...
def test_decompression():
    if __name__ == '__main__':
        ...
        result = Compressor().decompress(1, test_input, 1568)
        ...
        for _ in range(1000000):
            Compressor().decompress(1, test_input, 1568)
        print(time.time() - t)
