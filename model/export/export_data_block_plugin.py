    ...
    def execute_internal(self, forge_reader: ForgeReader, forge_readers: list[ForgeReader], forge_data: ForgeData,
        ...
        for data_block_entry_id in data_block_files:
            ...
            if entry_file_data.type in (0x0984415E, 0x3F742D26):  # entity and entity group
                ...
                visual_resource_files: list[BaseFile] = list(filter(lambda x: x.resource_type == 0xEC658D29, valid_entity_nested_files))
                ...
                for visual_file in visual_resource_files:
                    ...
                    if 0x01437462 in visual_file.nested_files.keys():  # LOD selector
                        ...
                        print(
                            f"Could not find mesh instance data for {entry_file_data.name} {entry_file_data.id} ({entry_file_data.id:016X})")
                        continue

                    if mesh_instance_data is None:
                        print(f"Failed to find mesh data inside entity: {entry_file_data.name}")
                        continue
